...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
	    ...
	    int counter = 0;
	    char eletter = 'e';
	    char testletter, prevChar = 0;
	    int lastLetter = newWord.length() - 1;
		for(int i = 0; i < newWord.length(); i++){
			testletter = newWord.charAt(i);
			if(i != 0){
				prevChar = newWord.charAt(i - 1);
			}			
	    	if(newWord.charAt(lastLetter) == eletter && counter == 0 && i == lastLetter){
	    		counter += 1;
	    	}
	    	else if(newWord.charAt(lastLetter) == eletter && counter != 0 && i == lastLetter){
	    		counter += 0;
	    	}
	...
	public double getFleschScore()
	{
	    ...
	    double returnVal;
	    double numWords = getNumWords() * 1.0;
	    double numSentences = getNumSentences() * 1.0;
	    double numSyllables = getNumSyllables() * 1.0;
	    
	    returnVal = 206.835 - (1.015 * (numWords/numSentences)) - (84.6 * (numSyllables / numWords));
